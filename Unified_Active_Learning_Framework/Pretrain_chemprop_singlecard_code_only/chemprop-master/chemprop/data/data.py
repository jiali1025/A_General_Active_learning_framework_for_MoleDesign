...
from chemprop.features import BatchMolGraph_mask, MolGraph_mask
...
CACHE_GRAPH = True
SMILES_TO_GRAPH: Dict[str, MolGraph_mask] = {}
...
def cache_graph() -> bool:
    r"""Returns whether :class:`~chemprop.features.MolGraph_mask`\ s will be cached."""
    return CACHE_GRAPH


def set_cache_graph(cache_graph: bool) -> None:
    r"""Sets whether :class:`~chemprop.features.MolGraph_mask`\ s will be cached."""
    global CACHE_GRAPH
    CACHE_GRAPH = cache_graph


def empty_cache():
    r"""Empties the cache of :class:`~chemprop.features.MolGraph_mask` and RDKit molecules."""
...
class MoleculeDatapoint:
    ...
    def __init__(self,
                 smiles: List[str],
                 extra_mask_idx: List[Union[int, float]],
        ...
        :param extra_mask_idx: A list of the mask atom idx for the molecules. (no need to do for multiple molecules)
        ...
        """
        self.smiles = smiles
        self.extra_mask_idx = extra_mask_idx # a list of mask atom idx
        self.targets = targets # make sure the order of the targets same as the order of the mask atom idx
        self.row = row
    ...
    def num_tasks(self) -> int:
        ...
        In our case fix to 1

        :return: The number of tasks.
        """
        return 1

    def num_mask_atoms(self) -> int:
...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph_mask]:
        r"""
        Constructs a :class:`~chemprop.features.BatchMolGraph_mask` with the graph featurization of all the molecules.

        .. note::
           The :class:`~chemprop.features.BatchMolGraph_mask` is cached in after the first time it is computed
           and is simply accessed upon subsequent calls to :meth:`batch_graph`. This means that if the underlying
           set of :class:`MoleculeDatapoint`\ s changes, then the returned :class:`~chemprop.features.BatchMolGraph_mask`
           will be incorrect for the underlying data.

        :return: A list of :class:`~chemprop.features.BatchMolGraph_mask` containing the graph featurization of all the
        ...
        if self._batch_graph is None:
            ...
            for d in self._data:
                ...
                for s, m in zip(d.smiles, d.mol):
                    if s in SMILES_TO_GRAPH:
                        ...
                        mol_graph = MolGraph_mask(m,False,0, d.extra_mask_idx, d.atom_features, d.bond_features,
            ...
            self._batch_graph = [BatchMolGraph_mask([g[i] for g in mol_graphs]) for i in range(len(mol_graphs[0]))]
    ...
    def targets(self) -> List[List[Optional[float]]]:
        ...
        """
        return [d.targets for d in self._data] # for pretrain [[1,23,4],[4,67,64,68]]
...
def construct_molecule_batch(data: List[MoleculeDatapoint]) -> MoleculeDataset:
    ...
    Additionally, precomputes the :class:`~chemprop.features.BatchMolGraph_mask` for the constructed
    ...
    data.batch_graph()  # Forces computation and caching of the BatchMolGraph_mask for the molecules
...
class MoleculeDataLoader(DataLoader):
    ...
    Now it is a distributed version.
    """
