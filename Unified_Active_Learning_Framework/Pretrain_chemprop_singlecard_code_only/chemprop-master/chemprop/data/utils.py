...
def get_data(path: str,
             smiles_columns: Union[str, List[str]] = None,
             mask_atom_columns: Union[str,List[str]] = None,
             target_columns: List[str] = None,
    ...
    :param smiles_columns: The names of the columns containing SMILES.
                           By default, uses the first :code:`number_of_molecules` columns.
    :param mask_atom_columns: The names of the columns containing the mask_atom_index_list for the molecule.
    ...
    if args is not None:
        ...
        mask_atom_columns = mask_atom_columns if mask_atom_columns is not None else args.mask_atom_columns
    ...
    with open(path) as f:
        ...
        all_smiles, all_targets, all_rows, all_features, all_phase_features, all_weights, all_gt, all_lt, all_mask_idx = [], [], [], [], [], [], [], [], []
        for i, row in enumerate(tqdm(reader)):
            smiles = [row[c] for c in smiles_columns]

            mask_atom_list = [ast.literal_eval(row[c]) for c in mask_atom_columns]
            ...
            all_mask_idx.append(mask_atom_list)
        ...
        data = MoleculeDataset([
            MoleculeDatapoint(
                smiles=smiles,
                extra_mask_idx=all_mask_idx[i][0],
                targets=targets[0],
