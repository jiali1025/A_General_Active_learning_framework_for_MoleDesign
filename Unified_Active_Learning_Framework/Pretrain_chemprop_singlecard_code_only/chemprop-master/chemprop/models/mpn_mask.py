...
from chemprop.features import  get_atom_fdim, get_bond_fdim,  BatchMolGraph_mask, mol2graph_mask
from chemprop.nn_utils import index_select_ND, get_activation_function


class MPNEncoder(nn.Module):
    """An :class:`MPNEncoder` is a message passing neural network for encoding a molecule for masking prediction.
        As a result, we need the node hidden according to the masked atoms rather than the molecule vec as the usual
        chemprop setting.
    """
    ...
    def forward(self,
        ...
        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, mask_scope = mol_graph.get_components(atom_messages=self.atom_messages)
        mask_scope = torch.tensor(mask_scope)
        f_atoms, f_bonds, a2b, b2a, b2revb, mask_scope = f_atoms.to(self.device), f_bonds.to(self.device), a2b.to(self.device), b2a.to(self.device), b2revb.to(self.device), mask_scope.to(self.device)
        ...
        # print(mask_scope)
        # Readout
        masked_node_vecs = torch.index_select(atom_hiddens,0,mask_scope).float()
        ...
        return masked_node_vecs  # num_masked_atoms x hidden
...
class MPN(nn.Module):
    ...
    def forward(self,
                batch: Union[List[List[str]], List[List[Chem.Mol]], List[List[Tuple[Chem.Mol, Chem.Mol]]], List[BatchMolGraph_mask]],
                extra_mask_idx_list_batch: List[List[List[Union[int, float]]]] = None,
        ...
        :param batch: A list of list of SMILES, a list of list of RDKit molecules, or a
                      ...
                      The outer list or BatchMolGraph_mask is of length :code:`num_molecules` (number of datapoints in batch),
        ...
        :return: A PyTorch tensor of shape :code:`(num_molecules, hidden_size)` containing the encoding of each molecule.
        """
        if type(batch[0]) != BatchMolGraph_mask:
            ...
            if self.atom_descriptors == 'feature':
                ...
                batch = [
                    mol2graph_mask(
                        mols=b,
                        extra_mask_idx_list=extra_mask_idx_list,
                    ...
                    for b, extra_mask_idx_list in zip(batch,extra_mask_idx_list_batch)
                ]
        ...
        if self.atom_descriptors == 'descriptor':
            ...
            if not self.reaction_solvent:
                 ...
                 # most of the cases only one batch graph is inputted
            else:
                 encodings = []
                 for ba in batch:
                     if ba.is_reaction:
        ...
        return output #for our case of one molecule then it is num_masked_atoms_in_a_batch x hidden
