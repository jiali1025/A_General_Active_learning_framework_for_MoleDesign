...
from .mpn_mask import MPN
from chemprop.args import TrainArgs
from chemprop.features import BatchMolGraph_mask
...
class MoleculeModel(nn.Module):
    ...
    def create_ffn(self, args: TrainArgs) -> None:
        ...
        if args.ffn_num_layers == 1:
            ...
            ] # will use this one for pretraining since we want the encoder to be pretrained not the prediction head
            # The output should be the dimension: num_masked_node * num_class. Since we only have one task, then it should
            # be num_masked_node * self.output_size
        else:
            ffn = [
                dropout,
    ...
    def forward(self,
        ...
        output = self.ffn(self.encoder(batch, features_batch, atom_descriptors_batch,
                                       atom_features_batch, bond_features_batch))
        #num_masked_atom * num_class

        if self.classification and not (self.training and self.no_training_normalization) and self.loss_function != 'dirichlet':
            output = self.sigmoid(output)
        if self.multiclass:
            output = output.reshape((output.shape[0], self.num_classes))  # num_masked_atoms_in_a_batch x num targets x num classes per target
            ...
            #For cross entropy loss, it is true no need to do soft max in training
            # Maybe need to add weights to the loss, as the class may be highly imbalanced.
            # num_masked_atoms_in_a_batch * 1 * num_class in our case
            # num_masked_atoms_in_a_batch *  softmax(number of classes) for evaluation
        ...
        return output #num_masked_atoms_in_a_batch * 1 * number of classes
