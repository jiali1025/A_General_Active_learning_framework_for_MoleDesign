...
def train(
    ...
    for batch in tqdm(data_loader, total=len(data_loader), leave=False):
        ...
        # target_batch [[1,23,4],[4,67,64,68]] # shape = (batch_size, various_size(num_of_mask_atom per mol))
        targets = [num for sublist in target_batch for num in sublist] # shape (batch * various_size) (num_masked_atoms_in_a_batch)
        mask_batch = [num for sublist in mask_batch for num in sublist]
        ...
        if args.target_weights is not None:
            ...
            target_weights = torch.ones(targets.shape[0])
        ...
        if args.loss_function == "mcc" and args.dataset_type == "classification":
            ...
            if args.loss_function == "dirichlet":
                loss = loss_func(preds, targets, args.evidential_regularization) * mask
            else:
                # target_losses = []
                # for target_index in range(preds.size(1)):
                #     target_loss = loss_func(preds[:, target_index, :], targets[:]).unsqueeze(1)
                #     target_losses.append(target_loss)
                # loss = torch.cat(target_losses, dim=1).to(torch_device) * mask
                # print(preds.shape)
                # print(targets)
                target_loss = loss_func(preds,targets)
                loss = target_loss.to(torch_device) * mask
