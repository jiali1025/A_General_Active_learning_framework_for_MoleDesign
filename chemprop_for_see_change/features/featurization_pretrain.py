...
class Featurization_parameters:
    ...
    def __init__(self) -> None:
        ...
        self.MASK_ATOM_PERCENT = 0.1
        self.BOND_DELETION_PERCENT = 0.1
        self.SUBGRAPH_REMOVAL_PERCENT = 0.1
...
class MolGraphPretrain(object):
    ...
    * :code:`b2br`: A mapping from f_bonds to real bonds in molecule recorded in targets.
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        self.extra_mask_idx_MAM = mask_atom_pre_extra_mask_idx
        ...
        self.extra_mask_idx_bond_deletion = mask_bond_pre_extra_mask_idx
    ...
    def masked_atom_pretraining(self):
        ...
        if self.mask_atom_auto:
            ...
            self.masked_atom_index = torch.randperm(len(self.f_atoms))[:int(len(self.f_atoms) * self.mask_atom_percentage)]
            ...
            self.f_bonds = [
                self.f_bonds[idx] if idx not in bond_index_to_be_masked else mask_bond_atom_features(self.f_bonds[idx]) for
            ...
            self.f_atoms = [
                self.f_atoms[idx] if idx not in self.extra_mask_idx_MAM else mask_atom_features(self.f_atoms[idx],overwrite_default_atom=self.overwrite_default_atom_features) for idx
                in range(len(self.f_atoms))]

            bond_index_to_be_masked = [i for i, val in enumerate(self.b2a) if val in self.extra_mask_idx_MAM]
            ...
            self.f_bonds = [
                ...
                mask_bond_atom_features(self.f_bonds[idx],overwrite_default_atom=self.overwrite_default_atom_features,
                                        overwrite_default_bond=self.overwrite_default_bond_features) for idx in range(len(self.f_bonds))]

            self.final_masked_atom_index = self.extra_mask_idx_MAM
    ...
    def bond_deletion_complete(self):
        ...
        if self.mask_bond_auto:
            ...
            self.masked_complete_bond_index = torch.randperm(number_of_undirectional_bonds)[:int(number_of_undirectional_bonds * self.mask_bond_percentage)]
            ...
            masked_directional_bond_index_flattened = [item for sublist in masked_directional_bond_index for item in sublist]
            ...
            self.a2b = [[bond for bond in bonds if bond not in masked_directional_bond_index_flattened] for bonds in self.a2b]
            ...
            self.b2a = ['del' if index in masked_directional_bond_index_flattened else value for index, value in enumerate(self.b2a)]
            ...
            self.masked_complete_bond_index = self.extra_mask_idx_bond_deletion
            # need to operate on f_bonds
            masked_directional_bond_index = self.b2br[self.masked_complete_bond_index]
            masked_directional_bond_index_flattened = [item for sublist in masked_directional_bond_index for item in sublist]
    ...
    def subgraph_deletion(self,center):
        ...
        self.f_atoms = [
            self.f_atoms[idx] if idx not in atom_idx_to_remove else mask_atom_features(self.f_atoms[idx], overwrite_default_atom=self.overwrite_default_atom_features)
...
def mol2graphMaskAtom(mols: Union[List[str], List[Chem.Mol], List[Tuple[Chem.Mol, Chem.Mol]]],
    ...
    MolGraphPretrain_MA_list = [MolGraphPretrain(mol, af, bf,
                                   ...
                                   in zip_longest(mols, atom_features_batch, bond_features_batch, mask_atom_pre_extra_mask_idx_batch)]
