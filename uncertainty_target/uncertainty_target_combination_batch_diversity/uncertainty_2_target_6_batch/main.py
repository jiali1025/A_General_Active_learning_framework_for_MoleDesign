...
from cut_and_pred_target_property import cut_and_pred_target_property
from cut_and_pred_uncertainty import cut_and_pred_uncertainty
import subprocess
...
def main():
    for i in range(start_round, total_round):
        ...
        if i <= 2:
            cut_and_pred_uncertainty(dt_tot, 30000, i, check_point_path, threshold_value, threshold_num)
            print(f'cut_and_pred_uncertainty for round{i} has been finished')
            time.sleep(10)
        if i > 2:
            decay_param = 25
            cut_and_pred_target_property(dt_tot, 30000, i, check_point_path, decay_param)
            print(f'cut_and_pred_target_property for round{i} has been finished')
            time.sleep(10)
