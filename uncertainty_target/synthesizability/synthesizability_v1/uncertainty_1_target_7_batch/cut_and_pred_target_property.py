def cut_and_pred_target_v2(dt_tot_csv, cut_len, round, check_point_path):
    ...
    pred_tot['A_score_emitter'] = np.exp(-np.abs(2-ratio)**2) + pred_tot['Synthesizability']
    pred_tot['A_score_sensitizer'] = np.exp(-np.abs(1/0.9-ratio)**2) + pred_tot['Synthesizability']
